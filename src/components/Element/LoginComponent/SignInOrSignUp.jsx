...
export const RegisterOrLoginE = (props) => {
    ...
    const { isLoggedIn, handleSignIn, handleSignUp, isLoading } = props;
            ...
            if (email && password && fullName) {
                ...
                if (!response && singupErrorRef.current) {
                    singupErrorRef.current.innerHTML =
                        "Email already in use. Please try with abother email !";
    ...
    return (
        <div className={styles.bodyContent + " mainBodyHeight"}>
            <div
                className={
                    styles.container +
                    (showSignUpScreen ? "" : " " + styles.active)
                }
                id="container"
            >
                <div className={styles.formContainer + " " + styles.signUp}>
                    <form id="signUpForm">
                        ...
                        />
                        <span
                            className={styles.errorMsgForm}
                            id="signUpErrorSpan"
                            ref={singupErrorRef}
                        ></span>
                        <button
                            onClick={(event) => signupWithEmailPassword(event)}
                        >
                            Sign Up
                        </button>
                    </form>
                ...
                <div className={styles.formContainer + " " + styles.signIn}>
                    <form id="loginForm">
                        ...
                        <span
                            ...
                            ref={singinErrorRef}
                        ></span>
                        <button onClick={(event) => loginWithEmail(event)}>
                            Sign In
                        </button>
                    </form>
                </div>
                <div className={styles.toggleContainer}>
                    <div className={styles.toggle}>
                        <div
                            className={
                                styles.togglePanel + " " + styles.toggleLeft
                            }
                        >
                            <h1>Welcome Back!</h1>
                            <p>
                        ...
                        <div
                            className={
                                styles.togglePanel + " " + styles.toggleRight
                            }
                        >
...
const mapDispatchToProps = (dispatch) => ({
    handleSignUp: (email, password, fullName) =>
        dispatch(handleSignUp({ email, password, fullName })),
    handleSignIn: (email, password) =>
        dispatch(handleSignIn({ email, password })),
});
export const RegisterOrLogin = connect(
    mapStateToProps,
    mapDispatchToProps
)(RegisterOrLoginE);
